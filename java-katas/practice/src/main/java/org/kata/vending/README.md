...
## Requirements
- Inserting a coin accumulates the session balance; coins are tracked individually, not just as a
  running sum.
- Selecting a product by its code returns a sealed `DispenseResult` — one of: `Dispensed`,
  `InsufficientFunds`, `OutOfStock`, `UnknownProduct`, or `CannotMakeChange`. Expected failure
  modes are never thrown as exceptions.
...
- Requesting a refund returns the exact coins the user inserted (not equivalents from the float)
...
- The machine must serve one user at a time — all operations are mutually exclusive (thread-safe).

## What you're given
- `Coin` — the denominations the machine understands.
- `Product` — an immutable product descriptor (code, name, price).
- `DispenseResult` — a sealed result type for a selection attempt, with variants `Dispensed`,
  `InsufficientFunds`, `OutOfStock`, `UnknownProduct`, and `CannotMakeChange`.
