...
public class AtomicStampedStack<E> {
    ...
    public void push(E item) {
        ...
        // Allocate the stamp-holder array once per call, outside the loop. The array is
        // populated by AtomicStampedReference.get() with the current stamp. Re-using it
        // across loop iterations is safe because each iteration overwrites it with the
        // fresh value from get().
        int[] stampHolder = new int[1];

        while (true) {
            // (1) Snapshot: read head reference AND stamp atomically.
            //     get(stampHolder) returns the current reference and writes the stamp into
            //     stampHolder[0] as a side effect. This is the only way to read both values
            //     consistently — reading them via separate calls could observe a mismatch if
            //     another CAS fires between the two reads.
            Node<E> oldHead = head.get(stampHolder);
            int oldStamp = stampHolder[0];

            // (2) Build the new node pointing at the current head.
            Node<E> newNode = new Node<>(item, oldHead);
            int newStamp = oldStamp + 1; // monotonically advance the stamp

            // (3) CAS: succeed only if the reference is still oldHead AND the stamp is still
            //     oldStamp. If another thread pushed or popped (incrementing the stamp) between
            //     our snapshot and here, even if the reference happens to be the same object,
            //     the stamp mismatch will cause the CAS to fail and we retry.
            if (head.compareAndSet(oldHead, newNode, oldStamp, newStamp)) {
                return; // commit succeeded
            }
            // CAS failed (either ref or stamp changed) — retry with fresh snapshot.
        }
    }
    ...
    public Optional<E> pop() {
        ...
        while (true) {
            // (1) Snapshot reference + stamp atomically.
            Node<E> oldHead = head.get(stampHolder);
            int oldStamp = stampHolder[0];

            if (oldHead == null) {
                // Stack is empty. Note: we do not CAS here, so we do not advance the stamp —
                // a no-op pop does not consume a generation number.
                return Optional.empty();
            }

            // (2) Read next from the immutable snapshot. Safe because Node.next is final.
            Node<E> newHead = oldHead.next;
            int newStamp = oldStamp + 1;

            // (3) CAS head from (oldHead, oldStamp) to (newHead, newStamp).
            //     The stamp check is the ABA guard: if another thread has executed any push
            //     or pop since our snapshot, oldStamp will not match the current stamp and
            //     the CAS will fail, forcing us to retry with the fresh state.
            if (head.compareAndSet(oldHead, newHead, oldStamp, newStamp)) {
                return Optional.of(oldHead.item);
            }
            // CAS failed — stamp or reference changed; retry.
        }
    }
    ...
    public boolean isEmpty() {
        // getReference() performs a volatile read of the embedded reference.
