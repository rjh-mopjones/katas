...
public class BoundedBlockingQueue<E> {
    ...
    public void put(E element) throws InterruptedException {
        ...
        try {
            // while loop, not if — handles both spurious wakeups and the case where another
            // producer consumed the freed slot before this thread was scheduled (see class Javadoc).
            while (size == capacity) {
                notFull.await();
            }

            // There is now guaranteed space (we re-checked in the while loop).
            buffer[tail] = element;
            // Advance tail circularly. The modulo wraps back to 0 after reaching capacity.
            tail = (tail + 1) % capacity;
            size++;

            // A consumer waiting on notEmpty can now take this item. Signal exactly one —
            // we added exactly one element, so waking one consumer is sufficient. Signalling
            // more would only cause them to re-block after checking size > 0.
            notEmpty.signal();
        } finally {
            // Unlock in finally: any exception inside the critical section must not leave the
            // lock held, or the queue becomes permanently unusable by all other threads.
            lock.unlock();
        }
    }
    ...
    public E take() throws InterruptedException {
        lock.lockInterruptibly();
        try {
            // while loop for the same reasons as put(): spurious wakeups and races.
            while (size == 0) {
                notEmpty.await();
            }

            // Safe unchecked cast: we only ever store E elements in buffer (enforced by put's
            // null check and the generic type parameter). The raw Object[] is required because
            // Java generics are erased — we cannot allocate an E[].
            E element = (E) buffer[head];
            // Null out the slot to allow GC of the element — important for long-lived queues
            // where stale references would prevent collection of large objects.
            buffer[head] = null;
            head = (head + 1) % capacity;
            size--;

            // We freed one slot — signal one waiting producer.
            notFull.signal();
            return element;
        } finally {
            lock.unlock();
