    ...
    public <T> T execute(Callable<T> action) throws Exception {
        ...
        for (int attempt = 1; attempt <= policy.maxAttempts(); attempt++) {
            try {
                ...
                // If this was the last attempt, break immediately — no point sleeping after
                // the final failure; the caller needs the exception as quickly as possible.
                if (attempt == policy.maxAttempts()) break;

                // Compute base delay for this attempt (1-indexed: attempt 1 → base*m^0 = base).
                long delayMs = policy.computeDelayMs(attempt);

                if (policy.jitter()) {
                    // Full jitter: sample uniformly in [0, delayMs]. If delayMs is 0, this
                    // correctly produces 0 (no sleep). nextDouble() is in [0.0, 1.0).
                    delayMs = (long) (random.nextDouble() * (delayMs + 1));
                }

                // The sleeper abstraction lets tests record delays without blocking. Production
                // uses Thread.sleep; tests capture the list and assert shape of the backoff curve.
                sleeper.accept(delayMs);
            }
        }

        // Re-throw the last exception. We know lastException is non-null because we only reach
        // this line when maxAttempts >= 1 and the action threw on every attempt.
        throw lastException;
    }
}
