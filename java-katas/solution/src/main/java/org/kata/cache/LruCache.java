...
public class LruCache<K, V> implements Cache<K, V> {
    ...
    public Optional<V> get(K key) {
        ...
        // Promote to most-recent. Unlink from current position, then re-insert just after head.
        moveToFront(node);
    ...
    public void put(K key, V value) {
        ...
        if (existing != null) {
            // Update in place. Reusing the node avoids an allocation and preserves the
            // node's list position before promotion.
            existing.value = value;
            moveToFront(existing);
            return;
        }

        // New key — check capacity before allocating.
    ...
    private void evictLru() {
        Node lru = tail.prev;
        // The only time tail.prev == head is when the cache is empty, which cannot happen
        // here because we check capacity > 0 and only call evictLru when size == capacity.
        unlink(lru);
        map.remove(lru.key);
    }
}
