...
public class VendingMachine {
    ...
    public synchronized void restock(Product product, int qty) {
        catalogue.put(product.code(), product);
        stock.merge(product.code(), qty, Integer::sum);
    }
    ...
    public synchronized void insertCoin(Coin coin) {
        insertedAmount = insertedAmount.add(coin.value());
    ...
    public synchronized DispenseResult select(String code) {
        ...
        if (p == null) {
            return refundAndReturn(new DispenseResult.UnknownProduct(code));
        }
        if (stock.getOrDefault(code, 0) <= 0) {
            return refundAndReturn(new DispenseResult.OutOfStock(code));
        }
        if (insertedAmount.compareTo(p.price()) < 0) {
            // Non-terminal: keep the session open so the user can insert more coins.
            return new DispenseResult.InsufficientFunds(p.price().subtract(insertedAmount));
        }

        // Pin the scale so change-owed has a canonical representation independent of how
        // insertedAmount was accumulated. HALF_EVEN avoids cumulative round-up bias.
        BigDecimal changeOwed = insertedAmount.subtract(p.price()).setScale(2, RoundingMode.HALF_EVEN);

        // Plan change against the post-acceptance inventory — the user's inserted coins are
        // physically inside the machine by now, so they're available for change-making. We
        // copy into a fresh EnumMap so planGreedyChange can mutate freely.
        Map<Coin, Integer> projectedInventory = new EnumMap<>(coinInventory);
        for (Coin c : insertedCoins) projectedInventory.merge(c, 1, Integer::sum);

        List<Coin> changeCoins = planGreedyChange(changeOwed, projectedInventory);
        if (changeCoins == null) {
            // Can't honour the transaction without short-changing the user — refund and bail
            // BEFORE any real-state mutation. This is the payoff of plan-then-commit.
            return refundAndReturn(new DispenseResult.CannotMakeChange(changeOwed));
        }

        // Commit phase. Everything below this line is the side-effect application of a plan
        // we've already validated. Order is intentional but order-independent under the lock.
        for (Coin c : insertedCoins) coinInventory.merge(c, 1, Integer::sum);
        for (Coin c : changeCoins) coinInventory.merge(c, -1, Integer::sum);
        stock.merge(code, -1, Integer::sum);
        resetSession();
        return new DispenseResult.Dispensed(p, changeCoins);
    }
    ...
    public synchronized List<Coin> refund() {
        List<Coin> refunded = List.copyOf(insertedCoins);
        resetSession();
        return refunded;
    }
    ...
    private List<Coin> planGreedyChange(BigDecimal amount, Map<Coin, Integer> projected) {
        ...
        return remaining.signum() == 0 ? coins : null;
    }
