...
// TestLFU_Concurrent_NoRace hammers Get/Put from many goroutines over an
// overlapping key space, with capacity < keyspace so evictions fire concurrently.
// Under `go test -race` this proves the baked-in Mutex guards the items map and
// frequency lists; an RWMutex on Get (or no lock) would be flagged. It also asserts
// no torn values.
func TestLFU_Concurrent_NoRace(t *testing.T) {
	c := NewLFU[int, int](8) // cap 8 < keyspace 16 -> eviction churn
